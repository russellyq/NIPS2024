...
class LaserScan:
  ...
  def open_scan_with_points(self, points, intensity):
    self.reset()   
    self.set_points(points=points.reshape(-1, 3), remissions=intensity.reshape(-1,))
  ...
  def do_range_projection(self):
    ...
    self.proj_x = proj_x.astype(np.float32)
    self.proj_y = proj_y.astype(np.float32)
