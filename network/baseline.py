...
class get_model(LightningBaseModel):
    def __init__(self, config, backbone="resnet34", pretrained=True):
...
class get_model_range(LightningBaseModel):
    def __init__(self, config, backbone="resnet34", pretrained=True, init=True):
        super(get_model_range, self).__init__(config)
        self.save_hyperparameters()
        self.input_dims = config['model_params']['input_dims']
        self.hiden_size = config['model_params']['hiden_size']
        self.num_classes = config['model_params']['num_classes']
        ...
        self.get_model = get_model(config, backbone, pretrained)
        # # range:
        # self.depth_backbone = ResNet_Model(backbone, pretrained, config, freeze=False)
        # self.range_backbone = ResNet_Model(backbone, pretrained, config, freeze=False)
        self.depth_backbone = SwimT_Model(backbone, pretrained, config, freeze=False)
        self.range_backbone = SwimT_Model(backbone, pretrained, config, freeze=False)
    ...
    def _init(self, init=False):
        if init:
            # # load pretrained 3d backbone:
            with open('/home/yanqiao/2DPASS/pretrained/model_3d.pth', "rb") as f:
    ...
    def forward(self, data_dict):
        with torch.no_grad():
            data_dict = self.get_model.voxelizer(data_dict)

        data_dict = self.get_model.voxel_3d_generator(data_dict)

        # # range-depth image backbone
        x = data_dict['proj_range']
        layer1_out, layer2_out,layer3_out,layer4_out = self.depth_backbone(x)
        range_depth_feats = [layer1_out,layer2_out,layer3_out,layer4_out]

        # range-xyz image backbone
        x = data_dict['proj_xyz']
        layer1_out, layer2_out,layer3_out,layer4_out = self.range_backbone(x)
        range_xyz_feats = [layer1_out,layer2_out,layer3_out,layer4_out]
        ...
        enc_feats = []
        for i in range(self.num_scales):
            enc_feat = self.get_model.spv_enc[i](data_dict)
            ...
            proj_range_feat = range_depth_feats[idx]
            proj_xyz_feat = range_xyz_feats[idx]


            range_feats, xyz_feats = [], []
            for batch_id in range(batch_size):
                x_batch = laser_x[torch.where(laser_x[:,0] == batch_id)][:, -1]
                y_batch = laser_y[torch.where(laser_y[:,0] == batch_id)][:, -1]
                range_feat = proj_range_feat[batch_id, :, y_batch.long(), x_batch.long()]
                xyz_feat = proj_xyz_feat[batch_id, :, y_batch.long(), x_batch.long()]

                range_feats.append(range_feat.permute(1, 0).contiguous())
                xyz_feats.append(xyz_feat.permute(1, 0).contiguous())

            range_feats = torch.cat(range_feats, 0)
            xyz_feats = torch.cat(xyz_feats, 0)

            # # feature learner: 
            feat_learner = F.relu(self.leaners[i](range_feats + xyz_feats))
            enc_feat = enc_feat + feat_learner
            
            data_dict['range_depth_layer_{}'.format(i)] = feat_learner

            enc_feats.append(enc_feat)


        output = torch.cat(enc_feats, dim=1)
        data_dict['logits'] = self.get_model.classifier(output)

        data_dict['loss'] = 0.
        data_dict = self.get_model.criterion(data_dict)

        return data_dict

class get_model_range_v2(LightningBaseModel):
    def __init__(self, config, backbone="resnet34", pretrained=True, init=True):
        super(get_model_range_v2, self).__init__(config)
        self.save_hyperparameters()
        self.input_dims = config['model_params']['input_dims']
        self.hiden_size = config['model_params']['hiden_size']
        self.num_classes = config['model_params']['num_classes']
